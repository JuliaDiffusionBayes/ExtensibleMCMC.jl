# [Estimating the mean of a bivariate Gaussian](@id tutorial_mean_of_bivariate_gsn)
****

> In this tutorial we will use the package [ExtensibleMCMC.jl](https://JuliaDiffusionBayes.github.io/ExtensibleMCMC.jl/dev) to estimate the mean of a bivariate Gaussian random variable from its 10 observations. We will define a `target law` and two methods: `set_parameters!` and `loglikelihood` that dispatch on our `target law`. We will then generate some data, show how to parameterize the Markov chain and run the sampler. We will run multiple different samplers, displaying the range of possible algorithms implemented in [ExtensibleMCMC.jl](https://JuliaDiffusionBayes.github.io/ExtensibleMCMC.jl/dev). Each time, we will graphically inspect the results.
...
[ExtensibleMCMC.jl](https://JuliaDiffusionBayes.github.io/ExtensibleMCMC.jl/dev) expects to have two functions implemented for the `target law`. The first one is a setter of new parameters
```julia
...
!!! note
    (a single update of coordinate `1`) + (a single update of coordinate `2`) == a single MCMC iteration
...
Of course, the higher the dimension of the proposals the more difficult it becomes to find a suitable covariance matrix for them. For instance, this is what happens if the covariance is not scaled suitably:
